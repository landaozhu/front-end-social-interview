...
1. 函数执行结束后局部变量会被销毁，无法留存状态；
2. 没有原生机制隔离私有变量，全局变量容易污染、被随意篡改。
借助闭包，我们可以在外部受控地读写函数内部私有变量，同时长久保存内部状态，实现基础模块化封装。
...
**不再提问**（2026-09-04 用户要求）：`for (var i…) { setTimeout(() => console.log(i)) }` 这类阅读追问永不出。闭包定义 / 泄漏 / `null` 回收仍考。
